...
私有包 `@deepseek-ai/dsh-experimental-tool-findme-compute` 在 `ctx.tools` 注册五个工具。每个工具调用已有 FindMe AI Hub Admin API Endpoint，并返回结构化 `{ data, trace_id }` 成功 Envelope。API 继续负责验证、Adapter 执行、持久化、网络策略与审计。
...
真实 Loader Composition 从 `cordis.yml` 启动插件，验证五个注册 Schema，执行带认证请求，证明凭据型输入会在传输前被拒绝，并证明有界 API 错误不会暴露 Admin Token 或原始错误字段。
